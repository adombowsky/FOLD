...
n <- nrow(y)
# fitting a GMM
#source("extra_R/loc_foldball.R")
#sourceCpp("extra_src/locnorm_D_arma.cpp")
S <- 35000 # iterations
B <- 1000 # burnin
L <- 30 # og = 30
...
c.B <- c.mv$cl
# FOLD
Delta <- comp_delta(theta=theta,p=d,n=n)
...
# plotting theta
theta_bar = rowMeans(array(unlist(theta), dim = c(n, 2*d + choose(d,2), length(theta))), dims = 2)[,1:d]
...
s <- 1000
theta_s <- array(unlist(theta), dim = c(n, 2*d + choose(d,2), length(theta)))[,1:d,s]
...
ctheta_samps <- get_ctheta_samps(theta=theta, omega=w_vals[3], p=d, max.k = max.k)
cb <- ctheta_ball(labs[3,],ctheta_samps)
# plot PSM
