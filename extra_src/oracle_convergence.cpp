...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
double Betapair(rowvec X_i, rowvec X_j, mat mu, cube Sigma, vec pi){
  int L = mu.n_rows;
  mat q = zeros(L, L);
  double f_i = pi(0)*dmv(X_i, mu.row(0), Sigma.slice(0))+
    pi(1)*dmv(X_i, mu.row(1), Sigma.slice(1))+
  ...
  for (int l = 0; l<(L-1); l++){
    for (int m = l+1; m<L; m++){
      q(l,m) = pi(l) * pi(m) * (dmv(X_i, mu.row(l), Sigma.slice(l)) * dmv(X_j, mu.row(m), Sigma.slice(m)) +
        dmv(X_i, mu.row(m), Sigma.slice(m)) * dmv(X_j, mu.row(l), Sigma.slice(l)))/(f_i * f_j);
    }
  }
  double B_ij = accu(q);
...
//[[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
mat makeBinderOracle(mat X, mat mu, cube Sigma, vec pi){
  int n = X.n_rows;
  mat Betastar = zeros(n,n);
  for (int i = 0; i<(n-1); i++){
    for (int j = i+1; j<n; j++){
      Betastar(i,j) = Betapair(X.row(i), X.row(j), mu, Sigma, pi);
    }
  }
  return(Betastar);
}
