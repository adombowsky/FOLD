...
```
# Introduction
This is the simulation study for the skew-symmetric mixture. In this example, we repeatedly sample from this mixture, then cluster the data using FOLD, VI, Binder's, and \texttt{mclust}. This implementation is done using the \texttt{foreach} package in order to speed up computation. If you just want to see the results of one replication from this method, see the file "nongauss_cluster.R" in the "rfuncts" folder.

# Preliminaries
```{r}
# r packages
source("rfuncts/nongaussmix.R")
source("rfuncts/mvnorm_gibbs.R")
source("rfuncts/mnorm_D_apply.R")
source("rfuncts/mergeloss.R")
source("rfuncts/nongauss_cluster.R")
